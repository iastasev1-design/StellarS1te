...
async function writeUsers(users) {
  for (const user of users) {
    // ЕБАНАЯ ПРОВЕРКА: если нет пароля - хуй его вставляем
    if (!user.passwordHash) {
      console.error('❌ У ЁБАНА user.passwordHash === null, ПРОПУСКАЮ:', user.username);
      continue;
    }
...
// ========== ЕБАНУТЫЙ КОСТЫЛЬ ДЛЯ ДОБАВЛЕНИЯ АДМИНА ==========
try {
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/v1/auth/register") {
      ...
      // ЕБАНУТАЯ ХУЙНЯ: ГАРАНТИРУЕМ, ЧТО ПАРОЛЬ НЕ NULL
      ...
      const user = {
        ...
        username,
        passwordHash: passwordHash, // СУКА, УБЕДИСЬ, ЧТО ТУТ НЕ NULL!
        name: username,
        email,
        group: USER_GROUP,
        createdAt: new Date().toISOString(),
