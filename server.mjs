...
`).catch(err => console.error('❌ Products table error:', err.message));
...
`).catch(err => console.error('❌ Keys table error:', err.message));
...
`).catch(err => console.error('❌ Orders table error:', err.message));
...
`).catch(err => console.error('❌ Promocodes table error:', err.message));
...
async function writeUsers(users) {
  // Обновляем каждого пользователя в БД
...
// ========== ИНИЦИАЛИЗАЦИЯ: ДОБАВЛЯЕМ ДЕФОЛТНЫХ ПОЛЬЗОВАТЕЛЕЙ И КЛЮЧИ ==========
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    // (Я сохранил их в полной версии кода выше)

    // Проверка бета-ключа для лаунчера
