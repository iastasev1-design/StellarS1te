...
async function writeUsers(users) {
  for (const user of users) {
    if (!user.passwordHash) {
      console.error('❌ У user.passwordHash === null, ПРОПУСКАЮ:', user.username);
      continue;
    }
    await dbQuery(
...
async function initializeDatabase() {
  try {
    ...
    // ✅ ПРАВИЛЬНОЕ ИМЯ ПОЛЯ: passwordHash (с большой буквы H)
    await dbQuery(`
    ...
    // ========== УДАЛЯЕМ СЛОМАННОГО mef ==========
    await dbQuery(`DELETE FROM users WHERE username = 'mef'`);
    console.log('🗑️ Пользователь mef удалён');
...
// ========== КОСТЫЛЬ ДЛЯ АДМИНА ==========
try {
    console.log('👀 ПРОВЕРЯЮ, ЕСТЬ ЛИ АДМИН...');
    const check = await dbQuery(`SELECT * FROM users WHERE username = 'yanacujy'`);
    
    if (check.rows.length === 0) {
        console.log('🔥 АДМИНА НЕТ, СОЗДАЮ...');
        await dbQuery(`
            INSERT INTO users (id, username, "passwordHash", name, email, "group", "createdAt", hwid, uid, banned, banReason, subscription)
            VALUES (
                'yanacujy',
                'yanacujy',
                '0e9b7938220a6ba9f066b6bc71dde72a6b91c6439eaed0b6d6c485edd3b5802e',
                'yanacujy',
                'gua1mis6@mail.ru',
                'Админ',
                NOW(),
...
// ========== ПРОВЕРКА И ИСПРАВЛЕНИЕ ПАРОЛЯ АДМИНА ==========
try {
    console.log('🔧 ПРОВЕРЯЮ ПАРОЛЬ АДМИНА...');
    const adminCheck = await dbQuery(`SELECT "passwordHash" FROM users WHERE username = 'yanacujy'`);
    if (adminCheck.rows.length > 0) {
        const currentHash = adminCheck.rows[0].passwordHash;
        const correctHash = '0e9b7938220a6ba9f066b6bc71dde72a6b91c6439eaed0b6d6c485edd3b5802e';
        if (currentHash !== correctHash) {
            console.log('🔧 ПАРОЛЬ НЕПРАВИЛЬНЫЙ, ОБНОВЛЯЮ...');
            await dbQuery(`
                UPDATE users 
                SET "passwordHash" = '0e9b7938220a6ba9f066b6bc71dde72a6b91c6439eaed0b6d6c485edd3b5802e' 
                WHERE username = 'yanacujy'
            `);
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/v1/auth/register") {
      ...
      if (!passwordHash) {
        sendJson(response, 500, { message: "Ошибка хэширования пароля" });
        return;
      }
    ...
    if (request.method === "POST" && url.pathname === "/api/v1/auth/check-beta-key") {
      ...
      if (foundKey.expires && foundKey.expires < Date.now()) {
        sendJson(response, 200, { valid: false, message: "Ключ истёк" });
        return;
      }
